...
$site->set_title($language->get('Edit Shared Password'));
$site->set_config('container-type', 'container');

$id = (int) $url->get_item();

if ($id == 0) {
	header('Location: ' . $config->get('address') . '/passwords/share/');
	exit;
}

$passwords_array = $passwords->get(array('id' => $id, 'shared_user_id' => $auth->get('id'), 'old' => 0, 'get_other_data' => true, 'global_or_null' => 0));
...
if (isset($_POST['save'])) {
	if (!empty($_POST['name'])) {
		if (!empty($_POST['password'])) {
			...
			header('Location: ' . $config->get('address') . '/passwords/viewshare/' . $id . '/');
			exit;
		}
		else {
		...
		<div class="col-md-3">
			<div class="well well-sm">
				...
				<div class="pull-right">
					<button type="submit" name="save" class="btn btn-primary"><?php echo safe_output($language->get('Save')); ?></button>  <a href="<?php echo $config->get('address'); ?>/passwords/viewshare/<?php echo (int) $password['id']; ?>/" class="btn btn-default"><?php echo safe_output($language->get('Cancel')); ?></a>
				</div>
