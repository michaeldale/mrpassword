...
if ($id != 0) {
	...
	$category_names 		= $categories->get(array('shared_user_id' => $auth->get('id'), 'id' => $id, 'limit' => 1, 'get_other_data' => true, 'global' => 0));
...
$paswd_array['shared_user_id'] 	= $auth->get('id');
$paswd_array['old'] 			= 0;
...
<div class="row">
	...
	<div class="col-md-3">
		<div class="well well-sm">
			<div class="pull-left">
				<h4>
					<?php if (isset($category_name)) { echo safe_output($category_name); } else { ?><?php echo $language->get('Shared Passwords'); ?><?php } ?> (<?php echo count($passwords_array); ?>)
				</h4>
			</div>

			<div class="clearfix"></div>

			<p><?php echo $language->get('This page displays all the passwords stored in the shared category.'); ?></p>
			<br />
		</div>
		...
		<div class="well well-sm">
			<form method="get" action="<?php echo safe_output($_SERVER['REQUEST_URI']); ?>">
				...
				<div class="pull-right">
					...
					<a href="<?php echo safe_output($config->get('address')); ?>/passwords/share/" class="btn btn-default"><?php echo $language->get('Clear'); ?></a>
				</div>
				<div class="clearfix"></div>
			</form>
		</div>
	...
	<div class="col-md-9">
		...
		<p>
		<?php if ($access_level == 2) { ?>
			<a href="<?php echo $config->get('address'); ?>/passwords/addshare/<?php if ($id != 0) echo (int) $id . '/'; ?>" class="btn btn-default btn-sm"><?php echo $language->get('Add'); ?></a>
		<?php } ?>
		<?php if ($id != 0) { ?>
			<?php if ($show_all) { ?>
				<a href="<?php echo $config->get('address'); ?>/passwords/share/<?php echo (int) $id; ?>/" class="btn btn-default btn-sm"><?php echo $language->get('Hide Passwords'); ?></a>
			<?php } else { ?>
				<a href="<?php echo $config->get('address'); ?>/passwords/share/<?php echo (int) $id; ?>/?action=show_all" class="btn btn-default btn-sm"><?php echo $language->get('Show Passwords'); ?></a>
			<?php } ?>
		<?php } else { ?>
			<?php if ($show_all) { ?>
				<a href="<?php echo $config->get('address'); ?>/passwords/share/" class="btn btn-default btn-sm"><?php echo $language->get('Hide Passwords'); ?></a>
			<?php } else { ?>
				<a href="<?php echo $config->get('address'); ?>/passwords/share/?action=show_all" class="btn btn-default btn-sm"><?php echo $language->get('Show Passwords'); ?></a>
			<?php } ?>
		<?php } ?>
		</p>
		...
		<div class="table-responsive">
			<table class="table table-striped">
				...
				<tr <?php if ($i % 2 == 0 ) { echo 'class="switch-1"'; } else { echo 'class="switch-2"'; }; ?>>
					<td class="centre"><a href="<?php echo $config->get('address'); ?>/passwords/viewshare/<?php echo (int) $password['id']; ?>/"><?php echo safe_output($password['name']); ?></a></td>
                    <td class="centre"><span class="content-as-copy-box"><?php echo safe_output($password['username']); ?></span></td>
                    <td class="centre" name="password<?php echo (int) $password['id']; ?>"><?php if ($show_all) { ?><span class="content-as-copy-box"><?php echo safe_output($encryption->decrypt($password['password'])); ?></span><?php } else { ?><a href="#password<?php echo (int) $password['id']; ?>" class="show_share_password" id="id-<?php echo (int) $password['id']; ?>"><?php echo $language->get('Show'); ?></a><?php } ?></td>
				</tr>
				<?php $i++; } ?>
			</table>
		</div>
